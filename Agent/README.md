...
全量核对的结果：**190 段预期输出逐段对上了实跑，不一致 0 段**；另有 145 段因为
本身就含随机 / 时效内容而声明为非确定、单独计数。
...
| **预期输出核对** | `nbtool.py verify --all` | 执行后逐段检查每个 `### 预期输出` 能否在**真实输出**里找到；汇总把「核对一致」与「已声明非确定而跳过」分开计 | **190 / 190 段一致**，不一致 0 段；另 145 段声明非确定（跳过比对），问题文件 0 个 |
| **零丢失审计** | `nbtool.py coverage` | 159 个源 `.py` 逐个映射到目标 notebook，按**整行指纹**核对代码行是否都进了 notebook | **159 / 159 达标**，孤儿 1（已登记排除） |

### 「预期输出」是怎么核的

不是人眼扫的，也不是拿记事本比对。`nbtool.py verify` 会**真的把这个 notebook 从头执行一遍**，
再把每段 `### 预期输出` 拿去实跑输出里找。判定口径：

- 比对的是**紧邻在上、且确实产出了输出的那个 code cell** 的输出；
- 比对方式是**按顺序的子序列**：围栏里每一行都必须在实跑输出里按同样的先后出现，
  实跑输出多出来的行不算问题（输出里本来就常混着时间戳、路径、对象地址）；
- 行首行尾空白与空行一律忽略，所以缩进差异不会误判。

三类结果分开计，不含糊在一起：

| 判定 | 含义 | 本次 |
|---|---|---|
| ✓ 核对一致 | 围栏里每一行都按顺序在实跑输出里找到了 | **190 段** |
| ⊘ 已声明非确定 | 这段的说明范围里写了「每次不同 / 时间戳 / 由模型决定」之类，按声明跳过字面比对 | 145 段 |
| ✗ 不一致 | 围栏里写了实跑里根本没有的内容 —— 错别字、改过的文案、没替换的占位符都会被抓住 | **0 段** |

> 「已声明非确定」不是免检。它换了个核法：核的是**结构和字段名**对不对，而不是字面值。
> 判据按**段**取（从这段自己的小标题到下一个标题之前），不是按整格 —— 一个 markdown 格里
> 同时放「确定性输出」和「模型输出」很常见，按整格判会让确定的那段一起蒙混过关。
