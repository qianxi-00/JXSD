...
| 欠账 | 一句话 |
|---|---|
| checkpoint / 跨运行恢复未接线 | ✅ **本轮已处理（把死旋钮变响 + 补齐启用路径；默认仍不启用，理由见后）**：① 新增 `default_checkpointer()`（进程内单例 `InMemorySaver`），docstring 明说**进程一停检查点就没了**、跨进程持久化不在本版能力范围；② `script/hitl_demo.py` 改用它（真机跑通：`update_config` 处中断 → `Command(resume=approve)` → 恢复出答复，退出码 0）；③ `build_production_agent(checkpointer=None)` 现在**构建时就打 WARNING**，点明 `thread_limit=80` 不会累计、HITL 审批无法跨请求恢复（有测试钉住：不传告警一次、传了不吵）；④ `tracing_config` / `invoke_slow_agent` 新增 `thread_id`，透传到 langgraph 的 `configurable.thread_id`，且与 Langfuse 的 `session_id` **分属两个字段**（混用会导致"改追踪标识就悄悄改了线程"），有端到端用例（假 agent 记录 invoke config）。<br>**为什么默认不启用线程**：RAG 的 agentic 线路是"**每次显式带 `history`**"的无状态设计，再叠一个会累积的线程 ⇒ 模型会同时看到"显式 history"与"检查点里的历史"**两份上下文**（重复烧 token、还可能自我矛盾）。所以线程是显式 opt-in，且调用方要自己保证 history 与线程既有消息不重叠。<br>**仍未做（有意）**：跨进程持久化 saver（`langgraph-checkpoint-postgres`/`-sqlite` 需配套建表与迁移） |
...
| `semantic_text` 未采用课案摘要模板 | **有意保留（不是漏做）**：我们用清洗后 OCR 全文；换模板前必须先保证证据文本取 `ocr_text`（已改）—— 摘要模板会丢原始措辞，而票据召回恰恰依赖票面原文（人名/票号/金额的写法）。真换就按 §5.4 A 的口径在 `RAG/data/eval_set.jsonl` 上重跑 recall 再动 |
| 发票票号 / 航司代码抽取覆盖不足 | ✅ **本轮已修（覆盖率有真机前后对照）**：<br>• **机票承运方**：100 张里 83 张票面**没有任何中文航司名** —— 唯一信号是航班号的两字码（实测缺口票里的码分布 `CZ 15 / CA 15 / HU 7 / SC 7 / MF 6 / MU 5 / JD 5 / ZH 1`）。新增「IATA 两字码 → 航司」反查（20 家常见航司表）+ 中文名/英文名别名归一（`中国国际航空公司`/`国航`/`AIR CHINA` → 同一个值），并容忍 OCR 把字母 O 认成数字 0（实测 `H0` → `HO`）。覆盖率 **17/100 → 82/100**（即可召回的 82 条机票里 **100%**；另 18 条是空 OCR）。归一后取值 13 家、无分裂组。查不到的码**保持 None 不猜**（错填的承运方会静默进 SQL 过滤与分组统计）<br>• **发票票号标签**：改认 `发票编号` 与 `发票号码` 两种写法。⚠ 诚实标注：本仓 100 张发票**全部**写"发票编号"（实测 100/100 抽到票号），所以"发票号码"这一支**没有真实样本覆盖、只有单测覆盖**<br>• **火车票仍缺 1 条票号，且有意不修**：99 条的票号形态完全一致（`T` + 14 位数字），缺口那条（`ticket_a0b0762c7686e149f3c483fc`）OCR 只抓到 139 字、票号区被识别成 `G358L99739` —— **文本里根本没有 T 开头的票号**。这是数据缺陷，不是抽取缺陷；按仓库一贯口径保持 None（猜出来的票号会静默进过滤与引用，比缺失危险得多） |
...
| **Agentic 的"检索策略选择"不可观测** | 课案流程图有个节点是"选择查询策略（直接/HyDE/子查询/回溯）"。基础①与融合④ 的改写策略在代码里（`llm/chat.py::rewrite_query`），会作为 `rewrite` 字段进事件流与评估记录；而 **② 是主 Agent 在模型内部选**（`agentic/` 对三套改写模板 0 命中），代码里**没有任何字段记录它选了哪种策略** —— 只能从 `detail["queries"]` 反推。这属能力对齐、可观测性缺口：想知道"这题走的是 HyDE 还是回溯"目前只能看它生成的检索文本。<br>**登记备查，当前按"不改签名"处理**：要么在提示词里要求模型在工具调用时自报策略（要改工具签名，偏离课案冻结的签名），要么从 Langfuse trace 的推理内容里提取（无需改签名） |
| 若干工程质量项 | ✅ **本轮已修**：① `tqdm` 补进 `pyproject.toml`（`>=4.66`）并同步 `uv.lock` 的两张表 —— 它原本只是别的包的传递依赖，`script/langfuse_evaluation.py` 却在用；② **16 个**文件的模块说明从 `import` 之后挪到文件首个语句（台账当时只数到 8 个脚本，其余 8 个在 `app/`、`core/`、`data_process/`），`__doc__` 实测已能取到，4 处"已知缺陷、本次不动位置"的过期自述一并更正；③ 两处 `limit=10000` 的静默截断（`embed_tickets.py`、`keyword_retrieval.py`）提成常量 `EMBED_QUERY_LIMIT`/`CORPUS_LIMIT` 并**取满即告警**（反向也验过：只取到 300 行时不告警，避免告警变成噪音） |
...
| T5 | 中 | 优化篇 791 / 1178（成本要和质量一起看） | ✅ **已修（b71fd1a）**：新增 `agentic/usage.py::TokenUsageCallback`（走 LangChain 的 `on_llm_end` 累加**整条链路**的用量：主 Agent + evidence-analyst 子代理 + 工具内的调用；`usage_metadata` 缺失时仍计数但标 `usage_missing`）。`answer_query_agentic` 返回 `tokens`/`cost`；评估脚本条目级出 `tokens_in`/`tokens_out`/`tokens_total`/`llm_calls`，运行级出 `batch_mean_*`（成本靠**已有自动聚合**，不重复声明）。真机一串 Agentic 问答：**9 次调用 / 48,735 token（34,559 命中提示缓存）** | 没填单价时 `cost=None`（刻意不内置价目表：价格会变、模型名还可能只是网关别名）⇒ 报告里**不出**成本指标，而不是报 0 |
| T6 | 中 | 优化篇 2238-2239（图谱健康度三指标 + 补全与人工审核闭环） | ✅ **本轮已修**：① 三指标落地为纯函数（`graph_rag/quality.py`）—— `isolated_node_rate`（孤点/总数，**自环仍算孤点**）、`normalized_duplicate_rate`（归一化重名的**代理**指标）、`link_decision_metrics`（标注集上的**链接准确率** + **误建重复节点率**，按课案口径区分 `false_create`/`false_link`/`target_mismatch`）；② `script/graph_health.py` 真库采集 + 与上次报告对比，孤点率/重复率 **+5 个百分点**即告警并以退出码 2 表达；③ `graph_rag/completion.py` + `script/graph_completion.py` 实现补全闭环：扫孤点 → 同类型优先 + 向量余弦挑候选 → LLM 提议（**只认候选池里的名字**）→ ≥0.8 直接落库 / 0.5~0.8 进 `RAG/data/graph_review_queue.jsonl` / <0.5 丢弃并计数；④ `review-list` / `review-ok` / `review-no` 人工裁决，批准才落库；⑤ `script/eval_graph_linking.py` 用**产线的** `builder.match_entity` 在人工标注集上算链接准确率。**三条诚实边界写进 README §4.12**：孤点率只能看趋势（真机基线就是 0/51，0 不等于健康）、重复率抓不到近义实体、0.8 这个自动阈值偏宽松 | 抽取/消歧退化会**静默污染图谱且没有可观测量**（现在有了孤点率/重名率/链接准确率三个读数） |
| T7 | 中 | 优化篇 2495-2497（`QueryResponse` + `response_model`） | ✅ **已修（b71fd1a + be67dfe）**：`graph_rag/service.py` 新增 `GraphQueryResponse`/`GraphSubgraph`/`GraphEntity`/`GraphRelationship`/`GraphCommunity`（全部 `extra="allow"`，图谱多给的字段照样透出），路由挂上 `response_model` 并按 `#/components/schemas/GraphQueryResponse` 进 OpenAPI。**可选字段一律 `None` + `response_model_exclude_none=True`**：图谱没给的字段就不出现在响应里（旧调用方按"缺字段跳过"写的载荷形状不变），`name` 反而收紧成必填（retriever 的每条 Cypher 投影都带 `n.name`，真缺了属于图谱数据损坏，500 带原文比静默返回匿名节点强） | OpenAPI 里现在有完整响应结构；`tests/test_graph_service_contract.py` 钉住字段与默认值语义 |
| T8 | 中 | 基础篇「接口职责」（`GET /api/health`、`WS /api/ws`、`QueryRequest.history`、`QA_CACHE_ENABLED`） | ✅ **本轮已修**：① `GET /api/health`（四依赖各探一次，**每个探针 3 秒上限**，任一失败只标 `degraded` 不返 5xx）；② `WS /api/ws`（与 SSE 同一套事件流，一条连接可连续多轮）；③ `ChatRequest.history`（四线路统一入口，①③ 单轮线路会忽略）；④ **`QA_CACHE_ENABLED` 总开关**（`config.py::QaCacheSettings`，一个开关管住四条线路：①透传 `run_and_collect(use_cache=)`、②叠加 `not history and cache_enabled()`、③④进 `_cached_route`）；⑤ SSE 每条载荷加 `query_type`（cache/faq/rag/rag_rejected/llm）与收尾帧 `processing_time`；⑥ CORS（**仅在配了 `APP_CORS_ORIGINS` 时挂**，默认不开放跨域）；⑦ 启动预热（`APP_WARMUP`，用 `AnswerCache.warmup()` 把**进程内预设矩阵**也载入，只播种 Redis 不算预热）。真机：`/api/health` 1.01s 四依赖全 ok；SSE 20 帧 token 全带 `query_type`、收尾 `processing_time=2.655s`；WS 一条连接连问两轮（第 2 轮 0.001s 命中缓存） | 接口层比课案薄：无健康检查、无 WebSocket、HTTP 不支持多轮、评估时无法一键关缓存 |
| T9 | 中 | 基础篇 1812-2274（12 字段口径） | `semantic_text` = 清洗后 OCR 全文，未采用课案三套摘要模板 | 召回语义依赖 OCR 原文措辞；若改模板，必须先保证证据文本取 `ocr_text`（已改）；**有意保留（不是漏做，同 B 表那条）**：摘要模板会丢原始措辞，而票据召回依赖票面原文（人名/票号/金额的写法）。真换就按 §5.4 A 的口径在 `RAG/data/eval_set.jsonl` 上重跑 Recall@K 再定 |
| T10 | 低 | 基础篇 3819-3911（FAQ 索引后端取舍） | FAQ 相似度层把向量全量载入进程内、每进程一份 | 当前 5 条标准问法无影响；问法涨到几百条要重评（`cache.py` 已写明该阈值）；**已知取舍，暂不动**：5 条标准问法下全量载入比走 Milvus 更快也更简单；问法涨到几百条时按 `core/cache.py` 里写明的阈值重评（那条注释就是这条台账的落点） |
...
| **checkpoint 与线程（本轮，B 表）** | `script/hitl_demo.py` 改用共享的 `default_checkpointer()` 后真机复跑：`[1] 发起运行 → ✅ 已暂停，待批工具 update_config{'key':'rerank_score_threshold','value':0.85} → [2] approve 恢复 → 出答复`，退出码 0。新增用例钉住三件事：不传 checkpointer 构建时**恰好告警一次**（且提到 `thread_limit`）、`default_checkpointer()` 是进程内**单例**（每次新建会让 thread 状态凭空消失）、`thread_id` 真的进到 `agent.invoke(config=...)` 的 `configurable` 里且不给时不凭空造 |
| **机票承运方覆盖率（本轮，B 表）** | 真机前后对照：`flight counterparty 17/100 → 82/100`（可召回的 82 条里 100%，另 18 条是空 OCR）。依据是先量后改：86/100 张票的 OCR 里**一个中文航司名都没有**（`中文航空: []`、`英文航司: []`），唯一信号是航班号两字码。归一后取值分布 13 家：`中国国际航空 30 / 中国南方航空 14 / 厦门航空 6 / 海南航空 6 / 山东航空 6 / 中国东方航空 5 / 首都航空 5 / 四川航空 3 / 华夏航空 2 / 云南祥鹏航空 2 / 深圳航空 1 / 吉祥航空 1 / 天津航空 1`（无分裂组）。重跑 `--insert` 后 `count(*) 300`、有向量 `282`、空文本 `18` 全部不变 |
...
| **接口层收尾复核（真机，本轮）** | 自起自收一次真服务（`RAG/app/main.py`，验完 `taskkill /F /T` 收树，8099 已释放）：① `GET /api/health` → `status=ok`、四依赖全 ok（`redis: True` / `milvus: collections=1` / `postgres: tickets=300` / `neo4j: connected`）、`qa_cache_enabled=true`、`warmup=true`；② 启动日志 `[预热] FAQ 相似度层 5 条 / 预设矩阵已载入 5 行` + `[BM25] 语料加载完成,共 300 篇 \| 耗时 0.52s`；③ SSE **缓存未命中**路径：37 个 `delta` 帧且**每帧带 `query_type=rag`**、收尾帧 `cache_hit=None`、`processing_time=2.562`、墙钟 2.59s（答案与标准答案一致）；④ SSE **缓存命中**路径：5 帧、`query_type=faq`、`processing_time=0.001`、`cache_hit=exact`；⑤ `WS /api/ws`：6 帧（5 token + 1 done）、`cache_hit=exact`、0.003s；⑥ `/api/modes` 返回四条线路（含 `supports_history` 标记）。**顺带记一个坑**：两种传输的帧形状**不同** —— SSE 是 `delta` / `done` 键（**没有** `type`），WS 是 `type` 键（token/done/error）。我第一版校验脚本按「SSE 也有 type」写，19 帧全被读成 `type=None`，差点误判成接口没实现；代码里两种形状都写着（`app/main.py` 的 SSE 分支与 WS 分支），写客户端时别照抄另一边 |
