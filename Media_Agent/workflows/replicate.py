...
本节要讲什么
    ...
    4. 视频链路之外还有**文章兜底**：课案的流程图写的是「下载视频/抓取文章」，
       但代码只实现了视频那一半。**拿不到文案时**（下载失败，或下到了却没转出文字）
       改用 ``fetch_article()`` 抓正文，知乎/公众号这类图文链接也能走完整条链路 ——
       后一种情况真实存在：yt-dlp 的 `html5` 抽取器会把「图文页里嵌了视频」的文章页
       认成可下载视频，下载成功但没人声，这时正文是唯一能救回文案的东西。

与课案的落地差异
    | 项 | 课案原文 | 本实现 | 原因 |
    |---|---|---|---|
    | 提取节点 | `download_video` → `extract_audio_text`，不判空 | 先 `extract_url` 洗一遍链接，视频链路**拿不到文案**时改走 `fetch_article` | 课案流程图本来就写了「下载视频/抓取文章」；拿不到文案时纯视频链路必然得到空串 |
    ...
    try:
        ...
        if video_path:
            ...
            if not text:
                ...
                # 没人声），先别急着判死 —— 下面的正文兜底仍会试一次（见那段注释）。
                print("[内容复刻] 视频已下载但转写为空（没人声 / ASR 报错），继续试正文兜底")
        else:
        ...
        # ⚠️ 判据是「**文案为空**」（`not text`），**不能**收窄成「下载就失败」
        #    （`not video_path`）：yt-dlp 的 `html5` 抽取器会把「图文页里嵌了 `<video>`
        #    的普通文章页」直接认成可下载视频（`download_video` 的选择器 `bv*+ba/b`
        #    对页面类型没有任何限制），于是「下载成功、但视频没人声或 ASR 报错」
        #    是一类**真实输入** —— 判据收窄就会把它直接判死，连正文都不试，
        #    而正文恰恰是这类输入唯一能救回文案的东西。
        #    代价：真「下载成功且确实没语音」时会多一次注定抓不到东西的 HTTP 请求
        #    （抓回空串后下面照样短路，无害）。一次白请求换「图文页内嵌视频」这类
        #    输入不整条链路白跑 —— 这个买卖划算，所以判据取 `not text`。
        if not text:
            print("[内容复刻] 改走文章抓取兜底...")
        ...
        # 下面这句话的前提是「视频与文章两条路都真的试过了」—— 判据是 `not text`
        # 时确实如此（转写为空也会去抓正文），所以「文章抓取也失败了」成立。
        if not text:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 3c2) 视频**下载成功、但转写为空** → 仍要走正文兜底（判据是「文案为空」）
        #      为什么判据不是「下载就失败」：yt-dlp 的 `html5` 抽取器会把「图文页里
        #      嵌了 `<video>` 的文章页」直接认成可下载视频，下载于是**成功**；
        #      而视频没人声、或 ASR 报错时转写为空 —— 这时正文是唯一能救回文案的东西，
        #      判据收窄就会把这类真实输入直接判死。
        #      代价：真「下载成功且确实没语音」时多一次注定抓不到东西的 HTTP 请求
        #      （抓回空串后照样短路，无害）。这个买卖划算。
        #      这条用例的文章桩**故意返回一段正文**：判据一旦被收窄回
        #      「只在下载失败时抓正文」，下面第一条断言立刻红。
        clear_calls()
        wire("C:/fake/demo.mp4", "", "一篇图文页里的真实正文（没人声时就靠它）")
        asr_empty = run_replicate("https://www.bilibili.com/video/BV1xx")
        assert calls["asr"] == ["C:/fake/demo.mp4"], calls["asr"]
        assert calls["article"] == ["https://www.bilibili.com/video/BV1xx"], \
            f"转写为空时应当走正文兜底，实际 calls['article']={calls['article']}"
        assert asr_empty["original_text"].startswith("一篇图文页里的真实正文"), \
            asr_empty["original_text"]
        assert len(calls["llm"]) == 3, calls["llm"]
        print("  ✓ 下载成功但转写为空：仍抓正文兜底 → 链路走完（3 次 LLM）")
