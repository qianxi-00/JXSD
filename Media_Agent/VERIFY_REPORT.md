...
| 抖音真实采集 | 服务**已部署并连通**、Cookie **已生效**（`/user/self` 解析成功、`handler_user_profile` 200）；**作品列表端点被 v4 镜像的上游签名缺陷挡住**（403，非 Cookie/非风控）。**降级入口（手动粘贴）已真跑验证通过**，详见第 8 节第 4 条 |
...
4. **抖音数据采集的真实拉取** —— **服务已部署连通、Cookie 已生效，但被上游的签名缺陷挡住**。
   ...
   **现在能用的路**：页面下方的「📋 手动粘贴作品数据」降级入口 ——
   后面的漏斗诊断 / 内容评估 / 优化策略三个 LLM 节点**完全一样**，只是数据要手工取。
   `fetch_user_videos()` 的失败信息现在会直接说明上述真因，不再误导成「Cookie 过期」。

   ✅ **这条降级路径本轮已重新真跑验证**（不是沿用早先记录）：
   造了一份「从表格复制粘贴」形态的 CSV（4 条作品 / 中文表头），
   走 `parse_manual_json()` → `run_review_from_json()`：

   ```
   [复盘] 手动数据解析完成: 4 条作品          ← 标题/播放/点赞/评论/分享/收藏 全部正确映射
   [复盘] 漏斗诊断中 ... 内容评估中 ... 生成优化策略中 ...
   elapsed = 162.3s
   funnel_diagnosis   : 逐条算出互动率并排序（22140 播放 / 12.46% 互动率 居首）
   content_assessment : 2903 字符，指出「救场案例流量强、模板类收藏高但播放低」
   suggestions        : 2875 字符，给出标题模板 / 脚本结构 / 发布时段
   三个节点全部有产出 ✓
   ```

   脚本：`.cache/e2e_review_manual.py`（含非空断言，避免"跑完了但没产出"被误判为通过）。
