...
flowchart TB
    ...
    V --> L2["第 2 层 · 视图层<br/>views/*.py 全部 import 成功<br/>+ 视图层断言（如 CDP 挑页级目标）"]
    V --> L3["第 3 层 · 环境契约<br/>配置可读 + 依赖 API 形状 + 关键文件<br/>+ 降级路径 + 文档键名契约 + 跨层平台契约"]
...
7. **加热点平台**（只对热点监控那条链路）—— 平台清单在**三个地方**各有一份，必须同时改：
   页面 `views/hot_topic.py` 的 `PLATFORM_OPTIONS`、工作流 `workflows/hot_topic.py` 的
   `PLATFORM_SENDS` 与 `FETCH_SOURCES`。漏改任何一处都是**静默给错数据**：
   只加在页面 → `route_fetch()` 会「回退到抖音」，用户选了新平台却拿到抖音热榜；
   只加在路由表 → `Send` 指向未注册节点，LangGraph **不报错**、只跳过该分支，
   页面最后显示「这次一条热点都没抓到（可能是热榜接口限流或被墙）」——
   把配置问题说成网络问题。第 3 层的「跨层契约」就是把这三张表互相钉住。
