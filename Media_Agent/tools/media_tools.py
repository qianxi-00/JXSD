...
def generate_image(prompt: str, output_path: str = None, size: str = "1024x1024") -> str:
    ...
    Note:
        ⚠️ **当前是没有 UI 入口的预留能力**：课案本身也没在页面上调用它，
        本项目六个页面同样没有入口 —— 全仓除自检外 0 个调用方。
        保留它的理由是给 ``MEDIA_IMAGE_*`` 三项配置一个落点，
        以及将来做图文类模块时能直接用；不是「漏接了一条链」。

        由此带来两个必须知道的事实：
        ① 本机 ``MEDIA_IMAGE_API_KEY`` / ``MEDIA_IMAGE_MODEL`` 都是空，
           所以**真实接口这条路从未用真 key 跑过**（状态是「实现了但未实测」），
           自检只覆盖占位图分支；
        ② 首页那张「图片生成」状态卡片反映的就是这两个键 —— 未配置时显示
           「未配置（用占位图）」，与这里的降级行为一致。
    """
    if output_path is None:
