...
def _to_duration_text(record: dict) -> str:
    ...
    **单位优先由「来源」判定，而不是只看数值大小**：
    ...
    Returns:
        str: ``"45秒"`` / ``"未知"`` / ``"不足1秒"``（原始值 > 0 但不足 0.5 秒）/
        原样返回的字符串（如 ``"1分30秒"``）。
    ...
    # 单位未知 → 才用课案的固定阈值。
    if not (ms_known or value >= 1000):
        return f"{value}秒"
    # 毫秒分支**四舍五入**（`round`），不用整除：整除会把不足 1 秒的片子一律压成
    # 「0秒」（500ms / 999ms 都得到 0），而上面 `value <= 0 → "未知"` 判的是**原始值**
    # —— 两个口径不一致时，明明有 0.5 秒的素材被显示成 0，看着像数据缺失。
    # `round()` 是银行家舍入（整半秒向偶数取整，如 2500ms → 2秒）：对「时长」这种
    # 展示值来说，半秒级误差不值得再写一套 half-up 舍入。
    seconds = round(value / 1000)
    # 原始值 > 0 但四舍五入后为 0（< 0.5 秒）：说「不足1秒」而不是「0秒」——
    # 后者等于把「有这条数据」说成「没有」。
    return f"{seconds}秒" if seconds > 0 else "不足1秒"
...
if __name__ == "__main__":
    ...
    #    （修复前 500ms 会被当成 500 秒，随后又一律整除成「0秒」）；
    #    第 8、9 条钉住「毫秒分支四舍五入」：整除时 999ms → 0秒、1500ms → 1秒，
    #    都是少报，且看起来像数据缺失。
    for rec, want, why in (
        ({"video": {"duration": 45000}}, "45秒", "嵌套 video = 抖音原始毫秒"),
        ({"video": {"duration": 500}}, "不足1秒", "嵌套 video 且 <0.5 秒：仍是毫秒，说不足 1 秒而不是 0 秒"),
        ({"duration_ms": 500}, "不足1秒", "键名带 _ms 后缀：恒为毫秒"),
        ...
        ({"video": {"duration": 999}}, "1秒", "四舍五入：999ms 进位成 1 秒（整除会给 0 秒）"),
        ({"video": {"duration": 1500}}, "2秒", "四舍五入：1500ms 进位成 2 秒（整除会给 1 秒）"),
    ...
    print("  时长单位判定             OK  12 种输入形状（嵌套/后缀/裸值/手写/四舍五入）")
